...
def move_external_axis_based_on_distance(R, dist_from_left_cm):
    """
    Move external axis 1 based on the detected distance from left (cm).
    """
    m1 = dist_from_left_cm
    if m1 < 9:
        value1 = 0
    elif m1 < 14.5:
        value1 = 0
        ...
        value1 = 0
    print(f"Moving external axis 1 to {value1} based on distance {m1:.2f} cm")
    R.set_external_axis([value1, 0, 0, 0, 0, 0])

# --- Main Logic ---

rect_data = objectdetection.detect_rectangle_from_camera()
if rect_data:
    print("Detected rectangle:", rect_data)
    dist = rect_data["dist_from_left_cm"]
    print("Distance from left (cm):", dist)
    move_external_axis_based_on_distance(R, dist)
else:
    print("No rectangle detected or user exited.")
